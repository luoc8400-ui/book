(() => {
  ...
  // 读取站点清单并渲染列表
  // 方法：loadManifest —— 使用稳定路径，避免 ?v= 导致 SW 缓存键不一致
  async function loadManifest() {
    ...
    if (!Array.isArray(files) || !files.length) {
      // 兜底：同时尝试“无空格”和“有空格”的中文文件名
      const candidates = [
        '帝霸1.txt', '帝霸2.txt', '帝霸3.txt', '帝霸4.txt',
        '帝霸 1.txt', '帝霸 2.txt', '帝霸 3.txt', '帝霸 4.txt'
      ];
      const found = [];
      for (const name of candidates) {
        ...
        try {
          ...
          if (head.ok) { found.push(name); continue; }
        } catch {}
        try {
      ...
      files = Array.from(new Set(found));
    }
